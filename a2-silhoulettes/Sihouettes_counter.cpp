...
Sihouettes_counter :: Sihouettes_counter() {
}
...
/* @param point - current point of the image, it is or a sihouelette or a canvas
 * @param width - width of the array
 * @param height  - the height of the array
 * @param imageLight - matrix of lightness
 * Does bypass of the sihoulette*/
void Sihouettes_counter :: fillSihoulette(QPoint &point, int** imageLight, int height, int width, int canvasColor) {
...
/* Return a count of the sihoulettes standing separately*/
int Sihouettes_counter :: count(string filePath) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (imageLight[i][j] != canvasColor) {
                ...
                point.setX(j);
                point.setY(i);
                fillSihoulette(point, imageLight, height, width, canvasColor);
                result++;
            }
        }
    }
